...
void codegen_cleanup_call_x86_64(CodegenContext *cg_context) {
    ...
    case FUNCTION_CALL_TYPE_INTERNAL:
        femit_x86_64(cg_context, I_ADD, IMMEDIATE_TO_REGISTER, (int64_t)(arch_data->current_call->call_arg_count * 8), REG_RSP);
        break;
...
(CodegenContext *cg_context,
 ...
 RegisterDescriptor target) {
  femit_x86_64(cg_context, I_LEA, NAME_TO_REGISTER,  REG_RIP, name, target);
}
...
(CodegenContext *cg_context,
 ...
 RegisterDescriptor target)  {
  femit_x86_64(cg_context, I_LEA, MEMORY_TO_REGISTER, REG_RBP, offset, target);
}
...
(CodegenContext *cg_context,
 const char *name,
 RegisterDescriptor target) {
  femit_x86_64(cg_context, I_MOV, NAME_TO_REGISTER, REG_RIP, name, target);
}
...
(CodegenContext *cg_context,
 long long int offset,
 RegisterDescriptor target)  {
  femit_x86_64(cg_context, I_MOV, MEMORY_TO_REGISTER, REG_RBP, offset,  target);
}
...
(CodegenContext *cg_context,
 ...
 const char *name) {
  femit_x86_64(cg_context, I_MOV, REGISTER_TO_NAME, source, REG_RIP, name);
}
...
(CodegenContext *cg_context,
 RegisterDescriptor source,
 long long int offset) {
  femit_x86_64(cg_context, I_MOV, REGISTER_TO_MEMORY, source, REG_RBP, offset);
}
...
(CodegenContext *cg_context,
 ...
 long long int immediate) {
  femit_x86_64(cg_context, I_ADD, IMMEDIATE_TO_REGISTER, immediate, reg);
}
...
void emit_instruction(CodegenContext *context, IRInstruction *instruction) {
    ...
    break;
    case IR_RETURN:
        // Restore all callee-saved registers used in the function.
...
int64_t x86_64_instruction_interference(IRInstruction* instruction) {
...
void codegen_emit_x86_64(CodegenContext *context) {
    ...
    (context, REG_RAX, GENERAL_REGISTER_COUNT, general, argument_register_count, argument_registers, x86_64_instruction_interference);
    ra(info);
